...
const TalentBoardPage = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <DecisionFields status={status} setStatus={setStatus} decision={decision} setDecision={setDecision} />
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {talents.map((talent) => (
          <div key={talent.id} className="card border p-4 mb-4">
            ...
            <p><strong>Venue Capacity:</strong> {talent.venueCapacity}</p>
            <p><strong>Agent:</strong> {talent.agent}</p>
            <p><strong>Agency:</strong> {talent.agency}</p>
            <p><strong>Status:</strong> {talent.status}</p>
            <p><strong>Review Notes:</strong> {talent.reviewNotes}</p>
          </div>
        ))}
      </div>
    </div>
